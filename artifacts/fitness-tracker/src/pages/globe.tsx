  ...
  return (
    <Layout headerVariant="black">
      <div className="relative bg-[#050508] text-white" style={{ height: "calc(100dvh - 3.5rem)" }}>
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex flex-col items-start gap-1.5 bg-gradient-to-b from-black/85 via-black/55 to-transparent px-6 pb-10 pt-6 sm:px-8 sm:pt-8">
          <h1 className="max-w-[min(90vw,28rem)] text-balance font-medium text-2xl tracking-tight text-white sm:text-3xl">
            Around the World <span className="text-primary">·</span> From Galway
          </h1>
          {data ? (
            <>
              <p className="label-mono text-[11px] uppercase tracking-[0.14em] text-white/70">
              ...
              {sportLegend.length > 0 && (
                <div className="flex flex-wrap gap-x-4 gap-y-1 pt-0.5">
                  {sportLegend.map((s) => (
                    <span key={s.name} className="label-mono flex items-center gap-1.5 text-[10px] uppercase tracking-[0.14em] text-white/70">
                      <span
        ...
        <div className="pointer-events-none absolute bottom-4 right-4 z-10 sm:bottom-6 sm:right-6">
          <p className="label-mono rounded-md bg-white/10 px-2.5 py-1.5 text-[10px] uppercase tracking-[0.14em] text-white/70 backdrop-blur-sm">
        ...
        {isPending ? (
          ...
          >
            ...
            <p className="text-sm text-white/70">Loading globe…</p>
          </div>
        ) : null}
